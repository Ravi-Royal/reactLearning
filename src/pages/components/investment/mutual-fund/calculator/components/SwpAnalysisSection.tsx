...
interface SwpAnalysisSectionProps {
...
export const SwpAnalysisSection = ({
  result,
  swpAmount,
  inflationRate,
  inflationStartFrom,
  ...
  const initialSwp =
    inflationStartFrom === 'sip-start' && yearsFromSipStart > 0 && parseFloat(inflationRate) > 0
  ...
  return (
    <>
                ...
                {inflationStartFrom === 'sip-start' && yearsFromSipStart > 0 && (
                  <div className="bg-amber-50 border border-amber-300 rounded p-2 mt-2 mb-2">
                    ...
                    <div className="text-xs text-amber-700 space-y-1">
                      <div>
                        • Your input: <span className="font-semibold">{formatCurrency(swp)}/month</span> represents the
                        value in <span className="font-semibold">today's money</span> (at SIP start)
                      </div>
                      <div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-xs">
                              • You entered:{' '}
                              <span className={`font-bold ${isOverSustainable ? 'text-red-700' : 'text-green-700'}`}>
                                {formatCurrency(currentSwpValue)}
                              </span>
                              {isOverSustainable && (
                                <span className="text-red-700">
                                  {' '}
                                  (Exceeds sustainable by {formatCurrency(currentSwpValue - sustainableInTodaysTerms)})
                                </span>
                              )}
                            </div>
                          </div>
      ...
      <div className="bg-teal-50 p-4 rounded-lg">
        ...
        {parseFloat(inflationRate) > 0 && (
          <>
            {inflationStartFrom === 'sip-start' && investYears > 0 && holdingYears >= 0 && (
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div>
                          • This grows to{' '}
                          <span className="font-semibold">{formatCurrency(result.minSWPToSustain)}/month</span> when SWP
                          starts (after {yearsFromSipStart} years)
                        </div>
                        ...
                        <div className="mt-2 pt-2 border-t border-blue-300">
                          ...
                          <div className="text-xs mt-1 italic">
                            This will be inflation-adjusted to {formatCurrency(result.minSWPToSustain)}/month when SWP
                            starts
                          </div>
                        </div>
                      </>
