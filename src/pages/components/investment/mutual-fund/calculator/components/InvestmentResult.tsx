...
import type { InvestmentResultProps } from './types';
import { SWPAnalysis } from './SWPAnalysis';

export const InvestmentResult: React.FC<InvestmentResultProps> = ({ result, formState }) => {
  const { postInvestmentHoldingPeriod, swpAmount, swpPeriod } = formState;
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 via-white to-teal-50 rounded-lg shadow-lg p-4 sm:p-6 border-2 border-green-200">
      ...
      <div className="space-y-4">
        ...
        <SWPAnalysis result={result} formState={formState} />
      </div>
    </div>
  );
};
