  ...
  const calculationResult = useMemo<CalculationResult>(() => {
    ...
    if (calculationMode === 'purchase') {
      ...
      const hasCurrentData =
        currentInputMode === 'price' ? currentQuantity && currentAvgPrice : currentQuantity && currentTotalPrice;

      const hasBuyData = buyInputMode === 'price' ? buyQuantity && buyPrice : buyQuantity && buyTotalPrice;
    ...
    <div className={RESPONSIVE_PATTERNS.padding.page}>
      ...
      <div className={RESPONSIVE_PATTERNS.margin.section}>
        ...
        <h1 className={`${RESPONSIVE_PATTERNS.text['3xl']} font-bold text-gray-800`}>
          {AVERAGE_CALCULATOR_TEXTS.PAGE_TITLE}
        </h1>
        <p className={`${RESPONSIVE_PATTERNS.text.base} text-gray-600 mt-1`}>
          {AVERAGE_CALCULATOR_TEXTS.PAGE_SUBTITLE}
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className={`bg-white rounded-lg shadow-md ${RESPONSIVE_PATTERNS.padding.cardLg} border border-gray-200`}>
          <div
            className={`flex flex-col sm:flex-row sm:items-center sm:justify-between ${RESPONSIVE_PATTERNS.margin.section} ${RESPONSIVE_PATTERNS.gap.sm}`}
          >
          ...
          <div
            className={`grid grid-cols-1 md:grid-cols-2 ${RESPONSIVE_PATTERNS.gap.lg} ${RESPONSIVE_PATTERNS.margin.section}`}
          >
            <div>
              <label
                htmlFor="currentQuantity"
                className={`block ${RESPONSIVE_PATTERNS.text.sm} font-medium text-gray-700 ${RESPONSIVE_PATTERNS.margin.element}`}
              >
              ...
              <input
                ...
                placeholder={AVERAGE_CALCULATOR_TEXTS.INITIAL_INVESTMENT.QUANTITY_PLACEHOLDER}
                min="0"
                step="1"
          ...
          <div
            className={`flex flex-col sm:flex-row sm:items-center sm:justify-between ${RESPONSIVE_PATTERNS.margin.section} border-t border-gray-200 pt-4 sm:pt-6 ${RESPONSIVE_PATTERNS.gap.sm}`}
          >
          ...
          {calculationMode === 'purchase' ? (
            <>
              <div
                className={`flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 ${RESPONSIVE_PATTERNS.gap.sm}`}
              >
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    placeholder={AVERAGE_CALCULATOR_TEXTS.ADDITIONAL_PURCHASES.QUANTITY_PLACEHOLDER}
                    min="0"
                    step="1"
                    className={`w-full ${RESPONSIVE_PATTERNS.padding.md} ${RESPONSIVE_PATTERNS.text.base} border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent`}
                  />
                ...
                {buyInputMode === 'price' ? (
                  ...
                  <div>
                    <label
                      htmlFor="buyTotalPrice"
                      className={`block ${RESPONSIVE_PATTERNS.text.sm} font-medium text-gray-700 ${RESPONSIVE_PATTERNS.margin.element}`}
                    >
                      Total Amount to Invest (₹){' '}
                      <span title="Total money you plan to spend on this new purchase.">ℹ️</span>
                    </label>
                    <input
                      type="number"
                      id="buyTotalPrice"
            ...
            <>
              <p className={`${RESPONSIVE_PATTERNS.text.sm} text-gray-600 mb-4`}>
                Enter target average price and current market price to calculate quantity needed
              </p>
              <div
                className={`grid grid-cols-1 md:grid-cols-2 ${RESPONSIVE_PATTERNS.gap.lg} ${RESPONSIVE_PATTERNS.margin.section}`}
              >
                ...
                <>
                  ...
                  <div className="flex justify-center mb-6">
                    <div className="bg-white border-2 border-indigo-200 rounded-xl shadow-lg px-8 py-6 flex flex-col items-center max-w-xs w-full">
                      ...
                      <div className="text-4xl font-extrabold text-indigo-700 mb-1">
                        {calculationResult.requiredQuantity.toFixed(0)} shares
                      </div>
                      <div className="text-gray-500 text-xs">
                        This is the number of shares you need to buy to reach your target average.
                      </div>
                    </div>
                  </div>
                </>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-center">
                <div className="bg-white rounded-lg shadow p-4 flex flex-col items-center">
                  <div className="text-gray-600 text-sm mb-1 flex items-center gap-1">
                    Total Quantity{' '}
                    <span title="Total shares after your new purchase." className="text-blue-400">
                      ℹ️
                    </span>
                  </div>
                  <div className="text-2xl font-bold text-gray-800">
                    {((parseFloat(currentQuantity) || 0) + calculationResult.requiredQuantity).toFixed(0)}
                  </div>
                </div>
                <div className="bg-white rounded-lg shadow p-4 flex flex-col items-center">
                  <div className="text-gray-600 text-sm mb-1 flex items-center gap-1">
                    Total Investment{' '}
                    <span title="Sum of your previous and new investments." className="text-blue-400">
                      ℹ️
                    </span>
                  </div>
                  <div className="text-2xl font-bold text-gray-800">
                ...
                <div className="bg-white rounded-lg shadow p-4 flex flex-col items-center">
                  <div className="text-indigo-700 text-sm mb-1 flex items-center gap-1">
                    New Purchase Amount{' '}
                    <span title="Amount needed for this new purchase only." className="text-blue-400">
                      ℹ️
                    </span>
                  </div>
                  <div className="text-2xl font-bold text-indigo-700">
                    ₹
                    {Money.multiply(
                      calculationResult.requiredQuantity || 0,
                      safeParseNumber(currentMarketPrice, 0),
                    ).toFixed(2)}
                  </div>
                </div>
                <div className="bg-orange-100 rounded-lg shadow p-4 flex flex-col items-center border-2 border-orange-300">
                  <div className="text-orange-600 text-sm font-medium mb-1 flex items-center gap-1">
                    New Average Price{' '}
                    <span title="Your new average price per share after this purchase." className="text-blue-400">
                      ℹ️
                    </span>
                  </div>
          ...
          {calculationMode === 'purchase' && calculationResult.newAvgPrice !== null && (
            <div className="bg-gradient-to-r from-orange-50 to-orange-100 border-2 border-orange-300 rounded-lg px-2 py-6 sm:py-8 mb-6">
              ...
              <div className="mt-4 text-center text-gray-700 text-base">
                After buying <span className="font-semibold">{safeParseNumber(buyQuantity, 0).toFixed(0)}</span> shares
                for ₹
                ...
                , your new average price will be{' '}
                <span className="font-semibold text-orange-600">₹{calculationResult.newAvgPrice.toFixed(2)}</span> for a
                total of{' '}
                <span className="font-semibold">
                  {Money.add(safeParseNumber(currentQuantity, 0), safeParseNumber(buyQuantity, 0)).toFixed(0)}
                </span>{' '}
                shares.
              </div>
            </div>
        ...
        <div
          className={`${RESPONSIVE_PATTERNS.margin.section} bg-blue-50 border border-blue-200 rounded-lg ${RESPONSIVE_PATTERNS.padding.card}`}
        >
          <h4
            className={`${RESPONSIVE_PATTERNS.text.sm} font-semibold text-blue-900 ${RESPONSIVE_PATTERNS.margin.element}`}
          >
            💡 How it works
          </h4>
          <p className={`${RESPONSIVE_PATTERNS.text.sm} text-blue-800 ${RESPONSIVE_PATTERNS.margin.element}`}>
            New Average Price = (Current Quantity × Current Avg Price + Buy Quantity × Buy Price) ÷ (Current Quantity +
            Buy Quantity)
          </p>
          <p className={`${RESPONSIVE_PATTERNS.text.xs} text-blue-700`}>
            <strong>Tip:</strong> Toggle between "Price" and "Total" mode to enter data based on average price per share
            or total amount invested.
          </p>
        </div>
      </div>
    </div>
