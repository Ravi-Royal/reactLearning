...
import {
  ...
  sortStockData,
  type SortDirection,
} from '../helpers/StockResult.helper';
import {
  pnlColumnKeys,
  StockColumnKey,
  stockColumns,
 ...
 */
const IGNORED_SYMBOLS = ['SGBDE31III-GB'];
...
const StockTable: React.FC<StockTableProps> = ({ stockData, priceMap }) => {
  ...
  return (
    <div className="-mx-4 sm:mx-0">
      <div
        className="shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg overflow-auto"
      ...
      >
        <table className="min-w-full border-collapse border border-gray-300 relative text-xs sm:text-sm">
          <thead className="bg-gray-100">
            <tr>
              {stockColumns.map(
                (col: { key: StockColumnKeyType; label: string; align?: 'left' | 'right' }, index: number) => (
                  <th
                    key={col.key}
          ...
          <tbody>
            {sortedData.map((row, index) => (
              <tr key={index} className="hover:bg-gray-50">
                {stockColumns.map(
                  (col: { key: StockColumnKeyType; label: string; align?: 'left' | 'right' }, colIndex: number) => {
                        ...
                        >
                          ...
                          {diff !== null && !isNaN(diff) && (
                            <span className={'ml-1 ' + diffColor}>
                              ({diff > 0 ? '+' : ''}
                              {diff.toFixed(2)} |{' '}
                              {diffTotal !== null && !isNaN(diffTotal)
                    ...
                    if (col.key === StockColumnKey.RealizedPriceVsValue) {
                      ...
                      return (
                        <td
                          ...
                          className={`border border-gray-300 px-2 sm:px-4 py-2${col.align === 'right' ? ' text-right' : ''} whitespace-nowrap${colIndex === 0 ? ' sticky left-0 bg-white z-10 relative shadow-[4px_0_8px_0_rgba(0,0,0,0.3)]' : ''}`}
                        >
                          <RealizedPriceVsValueIndicator
                            price={price as number | null}
                            buyPerStock={buyPerStock as number | null}
                            sellPerStock={sellPerStock as number | null}
                          />
                          {colIndex === 0 && (
                            <div className="absolute top-0 right-0 h-full w-[2px] bg-gray-300 z-40 pointer-events-none" />
                          )}
                    ...
                    if (col.key === StockColumnKey.UnrealizedPriceToCmp) {
                      ...
                      return (
                        <td
                          ...
                          className={`border border-gray-300 px-2 sm:px-4 py-2${col.align === 'right' ? ' text-right' : ''} whitespace-nowrap${colIndex === 0 ? ' sticky left-0 bg-white z-10 relative shadow-[4px_0_8px_0_rgba(0,0,0,0.3)]' : ''}`}
                        >
                          <UnrealizedPriceToCmpIndicator
                            currentPrice={currentPrice as number | null}
                            openValue={openValue as number | null}
                            openQuantity={openQuantity as number | null}
                          />
                          {colIndex === 0 && (
                            <div className="absolute top-0 right-0 h-full w-[2px] bg-gray-300 z-40 pointer-events-none" />
                          )}
