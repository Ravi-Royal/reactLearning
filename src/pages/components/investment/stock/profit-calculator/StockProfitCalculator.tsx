...
function StockProfitCalculator() {
  ...
  const updateStockGroup = (id: number, field: keyof StockGroup, value: string) => {
    setStockGroups(stockGroups.map((group) => (group.id === id ? { ...group, [field]: value } : group)));
  };
  ...
  const computeXIRR = useMemo(() => {
    ...
    const npv = (rate: number) =>
      flows.reduce((s, f) => Money.add(s, Money.divide(f.amount, Money.pow(1 + rate, f.years))), 0);
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md ${RESPONSIVE_PATTERNS.padding.cardLg} border border-gray-200`}>
      ...
      <h1 className={`${RESPONSIVE_PATTERNS.text.xl} font-bold text-gray-800 mb-2`}>
        {STOCK_PROFIT_CALCULATOR.HEADING}
      </h1>
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2
                className={`${RESPONSIVE_PATTERNS.text.lg} font-semibold bg-gradient-to-r from-gray-600 to-blue-600 bg-clip-text text-transparent`}
              >
                📊 Stock Group {index + 1}
              </h2>
              ...
              <div className="bg-white rounded-lg shadow-lg p-4 border-2 border-gray-300 relative">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className={`text-xl font-bold ${result.totalProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <div className="text-center">
                    ...
                    <div
                      className={`text-2xl font-bold ${result.profitPercentage >= 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                  ...
                  <div className="text-center">
                    ...
                    <div
                      className={`text-2xl font-bold ${result.annualizedReturn !== null && result.annualizedReturn >= 0 ? 'text-green-600' : result.annualizedReturn !== null ? 'text-red-600' : 'text-gray-400'}`}
                    >
      ...
      {stockGroups.length > 1 && overallTotals.totalInvested > 0 && (
        <div
          className={`${RESPONSIVE_PATTERNS.margin.section} bg-gradient-to-r from-purple-50 to-indigo-50 border-2 border-purple-300 rounded-lg ${RESPONSIVE_PATTERNS.padding.card}`}
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-lg shadow p-4 text-center">
              ...
              <div
                className={`text-2xl font-bold ${overallTotals.totalProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
            ...
            <div className="bg-white rounded-lg shadow p-4 text-center">
              ...
              <div
                className={`text-2xl font-bold ${overallXIRR !== null && overallXIRR >= 0 ? 'text-green-600' : overallXIRR !== null ? 'text-red-600' : 'text-gray-400'}`}
              >
      ...
      >
        ...
        <p className={`${RESPONSIVE_PATTERNS.text.xs} text-blue-700`}>
          <strong>Example:</strong> If you bought 100 stocks at ₹250 each (₹25,000 invested) and made a profit of
          ₹5,000, your profit percentage is 20%.
        </p>
        <p className={`${RESPONSIVE_PATTERNS.text.xs} text-blue-700 mt-2`}>
          <strong>Tip:</strong> Add multiple stock groups to calculate profit percentages for different purchases and
          see an overall summary. Enter the holding period to see annualized returns.
        </p>
      </div>
